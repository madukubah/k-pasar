...
class Blog extends User_Controller {
	...
	private $current_page = 'uadmin/blog/';
	
	public function __construct(){
		parent::__construct();
		$this->load->library('services/Blog_services');
		$this->services = new Blog_services;
		$this->load->model(
			array(
				'blog_model',
			)
		);
		$this->data[ "menu_list_id" ] = "blog_index";
	}
	public function index( $id_user = NULL )
	{
		...
		// echo $page;return;		 
		 ...
		 $pagination['total_records'] = $this->blog_model->record_count() ;
		...
		$table[ "rows" ] = $this->blog_model->blogs(  $pagination['start_record'], $pagination['limit_per_page'] )->result();
		$table = $this->load->view('templates/tables/plain_table_image', $table, true);
		$this->data[ "contents" ] = $table;

		// echo var_dump( $this->blog_model->db );return;
		$link_add = 
		array(
			"name" => "Buat Berita",
			"type" => "link",
	...
	public function create()
    {
        ...
        if ($this->form_validation->run()  )
        {
			...
			if( $this->blog_model->create( $data ) ){
				$this->session->set_flashdata('alert', $this->alert->set_alert( Alert::SUCCESS, $this->blog_model->messages() ) );
			}else{
				$this->session->set_flashdata('alert', $this->alert->set_alert( Alert::DANGER, $this->blog_model->errors() ) );
			}
        ...
        {
            $this->data['message'] = (validation_errors() ? validation_errors() : ($this->blog_model->errors() ? $this->blog_model->errors() : $this->session->flashdata('message')));
            if(  !empty( validation_errors() ) || $this->blog_model->errors() ) $this->session->set_flashdata('alert', $this->alert->set_alert( Alert::DANGER, $this->data['message'] ) );
            ...
            $this->render( "uadmin/blog/plain_content_form" );
        }
	}
	...
	public function edit( $blog_id = NULL )
	{
        ...
        if ($this->form_validation->run()  )
        {
			...
			if( $this->blog_model->update( $data, $data_param ) ){
				$this->session->set_flashdata('alert', $this->alert->set_alert( Alert::SUCCESS, $this->blog_model->messages() ) );
			}else{
				$this->session->set_flashdata('alert', $this->alert->set_alert( Alert::DANGER, $this->blog_model->errors() ) );
			}
        ...
        else
        {
            ...
            $form_data = $this->services->get_form_data( $blog_id );
            $form_data = $this->load->view('templates/form/plain_form', $form_data , TRUE ) ;
	...
	public function delete(  ) {
		...
		$data_param['id'] 	= $this->input->post('id');
		if( $this->blog_model->delete( $data_param ) )
		{
			// delete file
			if( !@unlink( $config['upload_path'].$this->input->post( 'file_content' ) ) )return;
			// delete image
